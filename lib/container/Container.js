...
import DependencyLocator from './dependencies/DependencyLocator';
...
export default class Container {
  ...
  constructor() {
    ...
    this.dependencyNameMappers = [];
    this.dependencyLocator = new DependencyLocator(this.injectableManager, this.dependencyNameMappers);
  }

  activate(...profileNames: Array<string>): Promise<Map<string, any>> {
    // Ensure that this container is not already active
    if (this.dependencyLocator.activeProfiles) {
      return PromiseHelper.wrapReject(new RueError('Container is already activated'));
    }
    this.dependencyLocator.activeProfiles = profileNames;
    ...
    let dependencyTraverser = new DependencyTraverser(this.dependencyLocator);
  ...
  getInstance(name: string): Promise<any> {
    let injectableEntry = this.dependencyLocator.resolve(name);
    if (!injectableEntry) {
  ...
  replaceInstance(name: string, replacement: any, parent: ?string): any {
    ...
    if (parent) {
      let injectableEntry = this.dependencyLocator.resolve(parent);
      if (!injectableEntry) {
