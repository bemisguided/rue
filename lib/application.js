...
function Application(registry, eventEmitter) {
  ...
  // Bind the root context to the application object
  var _rootContext = _getContext(ROOT_CONTEXT);
  Object.keys(_rootContext).forEach(function(name) {
    if (typeof _rootContext[name] === 'function') {
      _self[name] = _rootContext[name];
    }
  });
  ...
  this.activate = function(profiles) {
    ...
    dependencies.forEach(function(nextDependency) {
      promise = promise.then(function(dependency) {
        ...
        if (_registry.exists('@config')) {
          scope.config = _registry.get('@config').instance;
        }
